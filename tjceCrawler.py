...
class TJCE(scrapy.Spider):
    name = 'TJCE'
    allowed_domains = ['esaj.tjce.jus.br/cpopg/open.do']

    def __init__(self, numero_processo=None, *args, **kwargs):
        super(TJCE, self).__init__(*args, **kwargs)
        if numero_processo:
            self.start_urls = [
                f'https://esaj.tjce.jus.br/cpopg/show.do?processo.codigo=01000590S0000&processo.foro=1&processo.numero={numero_processo}'
            ] 
        else:
            self.start_urls = []
        self.results = []
